...
This appendix documents the hardware configuration parameters for the electromagnetic solenoid actuation system. These values represent the empirically determined results through the iterative testing programme described in Chapter~\ref{Chapter5}.
...
Shutter speed        & 3000        & Set to limit light entering the camera sensor \\
White balance        & Fixed matrix (manual)    & Calibrated against colour reference chart. \\
Top illumination     & WS2812B RGB LED array    & Addressable and can produce over 16M colours \\
Bottom illumination  & LCD backlight (dynamic)  & initially used for small prototype \\
Lux level & [To be specified]        & Tuned to minimise specular highlights. \\
\bottomrule
\end{tabular}
\end{table}

\vspace{1em}
\noindent Parameters marked ``[To be specified]'' are to be completed following the finalisation of the imaging calibration protocol at field procurement sites.
